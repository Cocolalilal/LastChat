...
- `PlatformMediaEncoder`: move the remaining `FileEncoder` image/video/audio base64 work behind a platform contract. Android can keep BitmapFactory/JPEG conversion behavior; iOS can use CoreGraphics/ImageIO and Foundation base64 without changing provider request JSON.
...
Android adapter seeds currently exist in `me.rerere.common.platform.android` for `PlatformHttpClient` and `PlatformFileStore`. The Android HTTP adapter owns OkHttp, SSE bridging, coroutine request awaiting, and HTTP proxy/proxy-auth wiring. New shared-candidate code should use the common contracts and receive these Android adapters through DI rather than importing OkHttp or `java.io.File` directly.
