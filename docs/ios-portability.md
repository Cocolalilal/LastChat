...
- `LocalToolPlatform`: keep local-tool helper behavior independent from JVM hashing, file existence checks, and Android gallery file persistence. Android currently owns SHA-256 sandbox suffixes and generated-image gallery writes through `AndroidLocalToolPlatform` / `AndroidGeneratedToolImageSaver`; iOS can provide native equivalents while preserving tool JSON and sandbox filenames.
