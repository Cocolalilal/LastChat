...
- `LocalToolPlatform`: keep local-tool helper behavior independent from JVM hashing, file existence checks, Android gallery file persistence, notification posting, notification listening, and WorkManager scheduling. Android currently owns SHA-256 sandbox suffixes, generated-image gallery writes, assistant notifications, recent-notification snapshots, and scheduled follow-up work through `AndroidLocalToolPlatform`, `AndroidGeneratedToolImageSaver`, and `AndroidLocalToolNotificationPlatform`; iOS can provide native equivalents while preserving tool JSON and sandbox filenames.
