...
- `MessageTemplateRenderer`: keep message-template transformation independent from Pebble, JVM `Reader`/`Writer`, and template-cache APIs. Both hosts share `renderMessageTemplate` / `PortableTemplateRuntime` (`message`/`role`/`time`/`date`, filters, comments, whitespace control, if/elseif/else). Android binds `PebblePortableTemplateRuntime` as an adapter that evaluates the template string and falls back to the portable engine. Remaining exotic Pebble syntax is Android-only.
...
- inline attachment audio now plays in-chat through `PlatformAttachmentAudioPlayer`
  ...
  transformers (placeholder, `renderMessageTemplate` / injected Pebble
  adapter, document, OCR, unsupported, workspace reminder). Android injects
  `PebblePortableTemplateRuntime`; iOS uses the same
  `renderMessageTemplate` engine (`{{ message }}`/`role`/`time`/`date`,
  filters, if/elseif, comments). iOS OCR is Vision `IosPlatformImageOcr`. Tool lists are
