 ...
 *
 * Currently only LiteRT is shipped. LiteRT primarily uses `.litertlm`, but the runtime
 * also accepts Gallery-style `.task` packages for models that still ship through the
 * MediaPipe/LiteRT task container. Adding another runtime later means adding a case here
 * + a Provider implementation + a tile in Settings, and nothing else has to change.
 */
@Serializable
