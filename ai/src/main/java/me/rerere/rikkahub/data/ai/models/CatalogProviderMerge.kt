...
fun mergeCatalogIntoProviders(
    ...
    val normalizedExisting = providers.map { provider ->
        ...
        val withCatalogIcon = if (catalogProvider != null) {
            ...
            provider.withCatalogManagedIcon(catalogProvider)
        } else {
            provider
        }
    ...
    return normalizedExisting + missingCatalogProviders.map(resolver::applyToProvider)
}
...
private fun ProviderSetting.withCatalogManagedIcon(
    ...
    return when (this) {
        is ProviderSetting.OpenAI -> copy(customIconUri = resolvedIcon)
