...
object DefaultPlaceholderProvider : PlaceholderProvider {
    override val placeholders: Map<String, PlaceholderInfo> = buildPlaceholders {
        ...
        placeholder("screen_brightness", { Text("Screen Brightness") }) {
            try {
        ...
        placeholder("volume_level", { Text("Volume Level") }) {
            try {
                val audioManager = it.context.getSystemService(Context.AUDIO_SERVICE) as android.media.AudioManager
                audioManager.getStreamVolume(android.media.AudioManager.STREAM_MUSIC).toString()
            } catch (e: Exception) {
                "unknown"
            }
        }

        placeholder("is_dark_mode", { Text("Is Dark Mode") }) {
            val nightModeFlags = it.context.resources.configuration.uiMode and android.content.res.Configuration.UI_MODE_NIGHT_MASK
            (nightModeFlags == android.content.res.Configuration.UI_MODE_NIGHT_YES).toString()
        }
        ...
        placeholder("is_music_active", { Text("Is Music Active") }) {
            val audioManager = it.context.getSystemService(Context.AUDIO_SERVICE) as android.media.AudioManager
            audioManager.isMusicActive.toString()
        }

        placeholder("weather", { Text("Current Weather") }) {
            try {
                // Get location first
                if (androidx.core.app.ActivityCompat.checkSelfPermission(
                        it.context,
                    ...
                    if (location != null) {
                        ...
                        // Parse JSON manually
                        val temp = response.substringAfter("\"temperature_2m\":").substringBefore(",").trim()
                        val weatherCodeStr = response.substringAfter("\"weather_code\":").substringBefore(",").substringBefore("}").trim()
